...
int main(void)
{
	...
	WyzBeePWM_Select_PWMPin(1);
	WyzBeePWM_Select_BaseTimer(1,Bt_IoMode0,Bt_PwmMode,&BT_TIMER(1));	//BT = 0 or BT =1 only can be selected

	WyzBeePWM_Init_Timer(&PWM_Timer, &BT_TIMER(1));

	WyzBeePWM_Config_DutyCycle(5,10, &BT_TIMER(1));							//frequency  in KHz and duty cycle in %

	WyzBeePWM_Config_InterruptCallback(&PWM_Interrupt, &PWM_Callback, &BT_TIMER(1));

	WyzBeePWM_Start_PWM(&BT_TIMER(1));

	Delay();					 											    // Delay some time

	WyzBeePWM_DisableInt(&BT_TIMER(1), &PWM_Interrupt); // Disable Interrupt
	Delay();
	WyzBeePWM_EnableSwTrig(&BT_TIMER(1));// Restart triggered by software
	m_u32CntIntTrg = 0;
	m_u32CntIntDuty = 0;
	m_u32CntIntUnder = 0;
	...
	{
		if (Bt_Pwm_GetIntFlag(&BT_TIMER(1), PwmTrigInt) == PdlSet)
		{
			Bt_Pwm_ClrIntFlag(&BT_TIMER(1), PwmTrigInt);
			m_u32CntIntTrg++;
		}
		else if (Bt_Pwm_GetIntFlag(&BT_TIMER(1), PwmDutyMatchInt) == PdlSet)
		{
			Bt_Pwm_ClrIntFlag(&BT_TIMER(1), PwmDutyMatchInt);
			m_u32CntIntDuty++;
		}
		else if (Bt_Pwm_GetIntFlag(&BT_TIMER(1), PwmUnderflowInt) == PdlSet)
		{
			Bt_Pwm_ClrIntFlag(&BT_TIMER(1), PwmUnderflowInt);
			m_u32CntIntUnder++;
		}
		u32Cnt++;
